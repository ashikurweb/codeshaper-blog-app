<header class="fixed left-0 top-0 w-full z-9999 py-7 lg:py-0":class="{ 'bg-dark/70 backdrop-blur-lg shadow !py-4 lg:!py-0 transition duration-100 before:absolute before:w-full before:h-[1px] before:bottom-0 before:left-0 before:features-row-border' : stickyMenu }"
  ...
  <div class="max-w-[1170px] mx-auto px-4 sm:px-8 xl:px-0 lg:flex items-center justify-between relative">
    <div class="w-full lg:w-1/4 flex items-center justify-between">
      <a href="{{ route('home') }}">
        <img src="{{ asset('frontend/pictures/logo.svg') }}" alt="Logo" class="w-32" />
      </a>
    ...
    <div class="w-full lg:w-3/4 h-0 lg:h-auto invisible lg:visible lg:flex items-center justify-between"
     ...
     <div class="relative">
        ...
        <div id="dropdownMenu" class="absolute right-0 mt-2 w-48 bg-gray-800 rounded-lg shadow-2xl opacity-0 scale-95 transition-all duration-300 ease-in-out hidden">
            <p class="block px-4 py-2 rounded-md border-b border-slate-900 bg-gray-700 transition-all duration-200 text-lime-500 hover:bg-gray-700">{{ Auth::user()->name }}</p>
            @auth
...
@push('scripts')
    <script>
        ...
        // Close dropdown when clicking on any link inside the dropdown
